...
function renderGallery() {
  ...
  grid.innerHTML = state.gallery.map((entry, i) => `
    <div class="gallery-item glass-card" data-gallery-index="${i}" style="animation-delay:${i * 0.06}s">
      <div class="gallery-item-header">
        <div class="gallery-item-name">${escHtml(entry.name || 'Untitled')}</div>
        <button class="gallery-remove-btn" type="button" aria-label="Remove ${escHtml(entry.name || 'design')} from Gallery" title="Remove from Gallery">×</button>
      </div>
...
function initGalleryInteractions() {
  ...
  grid.addEventListener('click', event => {
    ...
    if (event.target.closest('.gallery-remove-btn')) {
